...
import SKULLRobot.modules.sql.feds_sql as sql
...
from SKULLRobot.modules.helper_funcs.string_handling import markdown_parser
...
def fed_info(update: Update, context: CallbackContext):
    ...
    text = "<b>?¹ï? Federation Information:</b>"
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "SKULLRobot_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="SKULLRobot_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "SKULLRobot_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="SKULLRobot_fbanned_users.csv",
