from SKULLRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from SKULLRobot import DRAGONS
...
async def SKULLRobot_is_admin(chat_id: int):
    status = False
    SKULLRobot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if SKULLRobot.id == user.id:
            status = True
            break
    return status
