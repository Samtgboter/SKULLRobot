...
from SKULLRobot.modules.helper_funcs.extraction import extract_user_and_text
from SKULLRobot.modules.helper_funcs.string_handling import extract_time
from SKULLRobot.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Bring an order from ©AuraXNetwork to fight a Demon disaster."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from ©AuraXNetwork to fight a Tiger disaster."
            )
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"AuraX Just Kicked A Shit! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
